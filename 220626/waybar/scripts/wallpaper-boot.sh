...
awww query | awk -F: '{print $2}' | xargs | tr ' ' '\n' | while read -r monitor; do
    ...
    if [ -f "$SAVED_WALL" ]; then
        ...
        awww img -o "$monitor" "$SAVED_WALL" --transition-type none
    else
        # Fallback if no specific save exists for this monitor
        awww img -o "$monitor" "$HOME/Pictures/wallpapers/ado2.jpg" --transition-type none
    fi
done
